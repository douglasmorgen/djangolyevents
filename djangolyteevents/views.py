...
def index(request):
    ...
    client_id = "4GQQFB6MUA5Y2RNBIQ55"
    client_secret = "C5JPYH3P6XGF5G6D5PEGXHLZTQPVQISY7VCIGWE5L7WQT4WFK5"
    ...
    # hide_end_date=models.BooleanField(default=True)
                ...
                if  Event.objects.filter(external_id=event['id']).exists() and event['is_free']==False:
                ...
                if event['id']=='53667810867' and Event.objects.filter(external_id=event['id']).exists():
                    #save to DB
                    #external_id
